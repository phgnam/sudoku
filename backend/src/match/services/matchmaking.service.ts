...
export class MatchmakingService {
  ...
  addToQueue(entry: Omit<QueueEntry, 'joinedAt' | 'searchRadius'>): QueueStatus {
    ...
    this.logger.log(`Player ${entry.playerName} (${entry.rating}) joined ${entry.difficulty} queue`);
  ...
  getQueueStatus(playerId: string): QueueStatus {
    const difficulty = this.playerQueue.get(playerId);
    if (!difficulty) {
      return { position: 0, estimatedWait: 0, searchRadius: INITIAL_SEARCH_RADIUS };
    }
    ...
    for (const [difficulty, queue] of this.queues) {
      ...
      for (let i = 0; i < sortedQueue.length; i++) {
        ...
        for (let j = i + 1; j < sortedQueue.length; j++) {
          ...
          const maxRadius = Math.max(player1.searchRadius, player2.searchRadius);
