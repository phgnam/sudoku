...
export function useBorderValidation({
   ...
   */
  const countBordersAtVertex = useCallback(
    (vRow: number, vCol: number): number => {
   ...
   */
  const getAffectedVertices = useCallback(
    (type: "h" | "v", row: number, col: number): AffectedVertex[] => {
      if (type === "h") {
   ...
   */
  const wouldCreate4Way = useCallback(
    (type: "h" | "v", row: number, col: number): boolean => {
      // Validate border indices
      if (type === "h" && !isValidHorizontalBorder(row, col, gridSize)) {
        return true; // Invalid border, block it
      }
      if (type === "v" && !isValidVerticalBorder(row, col, gridSize)) {
        return true; // Invalid border, block it
      }
   ...
   */
  const canToggleBorder = useCallback(
    (type: "h" | "v", row: number, col: number): ToggleResult => {
      // Check if border is currently active
      const isCurrentlyActive =
        type === "h"
          ? (horizontalBorders[row]?.[col] ?? false)
   ...
   */
  const getBorderToggleability = useCallback(
    (
      type: "h" | "v",
      row: number,
      col: number,
    ): "can_toggle_on" | "can_toggle_off" | "blocked" => {
      const isCurrentlyActive =
        type === "h"
          ? (horizontalBorders[row]?.[col] ?? false)
          : (verticalBorders[row]?.[col] ?? false);
